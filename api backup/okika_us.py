...
import ctypes
from guizero import App, Window, Drawing, Text, TextBox, Picture, Box, MenuBar, PushButton
...
def_RX_LNA_gain = "15.0"

#filters
def_Rx_filterFB_freq = default_ultrasound_freq+5
def_Rx_filterFB_gain = "5.0"
def_Rx_filterFB_qual = "0.707"

def_Rx_filter0_freq = default_ultrasound_freq
def_Rx_filter0_gain = "5.0"
def_Rx_filter0_qual = "0.707"

def_Rx_filter1_freq = default_ultrasound_freq
def_Rx_filter1_gain = "15.0"
def_Rx_filter1_qual = "0.707"



#detector (slicer) level in volts
def_Rx_Slicer_Comp = "1.0"

max_range=8000 #max range of system in cm
...
def update_config(key): #gets the reconfiguration values in the Okika parameter text box values (see ghe guizero definitions below(
    ...
    if key==True:
        ...
        new_rx_f0_q = .707
        if( (new_rx_f0_freq is not '') and (new_rx_f0_gain is not '') and (new_rx_f0_q is not '')):
        ...
        new_rx_f1_q = .707
        if( (new_rx_f1_gain is not '') and (new_rx_f1_gain is not '') and (new_rx_f1_q is not '')):
        ...
        if(( new_rx_FB_freq is not '') and (new_rx_FB_gain is not '')):  
        ...
        if( new_rx_slicer_V is not ''): api.an_setGainInv(an_FPAA1_SlLvl,  ctypes.c_double(float(new_rx_slicer_V))) 
...
app = App(title="Okika FPAA Multi-Channel Ultrasonic Range Finder Example", width=800, height=700,layout="grid")
app.bg = "white"
app.file_name=""
...
LNA_text = Text(LNA_box,width=25, text="Rx LNA Settings",color="green",grid=[0,0],align="right")
...
slicer_in = Text(rx_slicer_box,width=25,size=10, text="Slicer Voltage, V (0.01~3.0", grid=[0,1],align="right")
...
#plot the meter marks.    
    for x in range(1,int((chart_width)/100),1):
        ruler_plot[ichrt].line(x*100*(1/mapsf),0,x*100*(1/mapsf),24,color="red",width=5 )    
    ...
    while closing_app == False:
        for istates in range(nchannels):
            ...
            if ((channels_on[istates] == True)):
                ...
                map_end_time = ref_ticks + map_duration #calculates the end time point of the plots referenced to current time
                currently_at = map_end_time - ref_ticks
                us_pulse()
                ...
                if((time_len < chart_width) and (time_len > 0)):  #calculate the location of markers and paint them in the channel's window
                    for echo_time in new_map_time:
                        ...
                        if xcoord <= chart_width:
                            new_id_item = map_plot[istates].line(xcoord,0,xcoord,99, color="black", width = "1")
